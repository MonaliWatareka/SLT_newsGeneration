...
export default function FileUpload({ onDocumentUploaded }) {
  ...
  const onDrop = useCallback(async files => {
    ...
    } catch (e) { toast.error('Upload failed: ' + (e.response?.data?.message || e.message)); }
  ...
  return (
    <div {...getRootProps()} className={`dz ${isDragActive ? 'dz-on' : ''} ${uploading ? 'dz-busy' : ''}`}>
      <input {...getInputProps()} />
      {uploading ? (
        <div className="dz-up">
          ...
          <div className="up-lbl">Ollama is processing your file…</div>
          <div className="prog"><div className="prog-fill" style={{ width: `${progress || 20}%` }} /></div>
          <div className="up-sub">llama3.2 / llava · may take 30–60s on CPU</div>
        </div>
      ) : (
        <div className="dz-idle">
          ...
          <div className="dz-h">{isDragActive ? 'Release to upload' : 'Drag & drop your file here'}</div>
          <div className="dz-s">or <span className="dz-a">click to browse</span> · PDF or Image · Max 20MB</div>
          <div className="dz-chips">
            <div className="dz-chip"><div className="cd cd-b" />PDF → llama3.2</div>
            <div className="dz-chip"><div className="cd cd-t" />Image → llava</div>
          </div>
        </div>
      )}
    </div>
