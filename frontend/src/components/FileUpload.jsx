...
export default function FileUpload({ onDocumentUploaded }) {
    ...
    const onDrop = useCallback(async (files) => {
        ...
        if (!allowed.includes(file.type)) { toast.error('Only PDF and image files supported'); return; }
        setUploading(true);
        try {
            const { data } = await uploadDocument(file, setProgress);
            toast.success('Summarized by Ollama!');
    ...
    const { getRootProps, getInputProps, isDragActive } = useDropzone({
        onDrop, accept: { 'application/pdf': [], 'image/*': [] },
        multiple: false, disabled: uploading,
    });

    return (
        <div {...getRootProps()} className={`dropzone ${isDragActive ? 'dz-active' : ''} ${uploading ? 'dz-uploading' : ''}`}>
            <input {...getInputProps()} />
            {uploading ? (
                <div className="dz-upload-content">
                    ...
                    <div className="upload-label">Processing with Ollama…</div>
                    <div className="prog-track">
                        <div className="prog-fill" style={{ width: `${progress || 15}%` }} />
                    </div>
                    <div className="upload-sublabel">llama3.2 / llava — may take 30–60s on CPU</div>
                </div>
            ) : (
                <div className="dz-idle-content">
                    <div className="dz-icon-cluster">
                        ...
                        <div className="dz-icon-divider">+</div>
                        <div className="dz-icon-box">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                    ...
                    <div className="dz-subline">or <span className="dz-link">click to browse</span> · PDF or Image · Max 20MB</div>
                    <div className="dz-chips">
                        <div className="dz-chip"><div className="chip-dot chip-dot-blue" />PDF → llama3.2</div>
                        <div className="dz-chip"><div className="chip-dot chip-dot-cyan" />Image → llava</div>
                    </div>
                </div>
            )}
        </div>
