...
export default function EmailSender({ newsletter }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast.success(`Sent to ${email}!`); setEmail('');
    } catch { toast.error('Email failed — check your Gmail App Password in .env'); }
    ...
    <div className="sender">
      {newsletter?.title && (
        <div className="ready-row">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="rr-ico">
      ...
      <div className="send-row">
        <div style={{ flex: 1 }}>
          ...
          <input type="email" className="finp" placeholder="recipient@example.com"
                 value={email} onChange={e => setEmail(e.target.value)} disabled={sending} />
        </div>
        <button className="btn-send" onClick={handleSend} disabled={sending} style={{ marginTop: 22 }}>
          {sending ? <div className="b-spin" /> : <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><line x1="22" y1="2" x2="11" y2="13" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/><polygon points="22 2 15 22 11 13 2 9 22 2" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
